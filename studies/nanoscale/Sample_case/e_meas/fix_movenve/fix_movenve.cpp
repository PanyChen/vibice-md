...
FixMovenve::FixMovenve(LAMMPS *lmp, int narg, char **arg) :
...
{
  if (narg < 4) error->all(FLERR, "Illegal fix movenve command");
  ...
  if (strcmp(arg[3], "linear") == 0) {
    if (narg < 7) error->all(FLERR, "Illegal fix movenve command");
    iarg = 7;
    mstyle = LINEAR;
    ...
    if (narg < 8) error->all(FLERR, "Illegal fix movenve command");
    iarg = 8;
    mstyle = WIGGLE;
    if (strcmp(arg[4], "NULL") == 0)
      axflag = 0;
    ...
    if (period <= 0.0) error->all(FLERR, "Illegal fix movenve command");

  } else if (strcmp(arg[3], "rotate") == 0) {
    if (narg < 11) error->all(FLERR, "Illegal fix movenve command");
    iarg = 11;
    mstyle = ROTATE;
    ...
    if (narg < 10) error->all(FLERR, "Illegal fix movenve command");
    iarg = 10;
    mstyle = VARIABLE;
    if (strcmp(arg[4], "NULL") == 0)
      xvarstr = nullptr;
      ...
      error->all(FLERR, "Illegal fix movenve command");
    if (strcmp(arg[5], "NULL") == 0)
      yvarstr = nullptr;
  ...
  while (iarg < narg) {
    if (strcmp(arg[iarg], "units") == 0) {
      if (iarg + 2 > narg) error->all(FLERR, "Illegal fix movenve command");
  ...
  FixMovenve::grow_arrays(atom->nmax);
...
void FixMovenve::initial_integrate(int /*vflag*/)
{
  int flag;
  ...
  if (mstyle == LINEAR) {
    ...
    double arg1 = omega_rotate * (delta-dt);
    double cosine1 = cos(arg1);
    double sine1 = sin(arg1);
    ...
    for (int i = 0; i < nlocal; i++) {
      if (mask[i] & groupbit) {
        ...
        if (axflag) {
          dtfm = dtf / mass[type[i]];
          if(delta !=dt) {
           v[i][0] -= ax*omega_rotate*cosine1;
          }
           v[i][0] += dtfm * f[i][0] + ax*omega_rotate*cosine;
           x[i][0] += (dtv * v[i][0]);
        } else if (rmass) {
          dtfm = dtf / rmass[i];
          v[i][0] += dtfm * f[i][0];
        ...
        if (ayflag) {
          dtfm = dtf / mass[type[i]];
          if(delta !=dt) {
           v[i][1] -= ay*omega_rotate*cosine1;
          }
           v[i][1] += dtfm * f[i][1] + ay*omega_rotate*cosine;
           x[i][1] += (dtv * v[i][1]);
        } else if (rmass) {
          dtfm = dtf / rmass[i];
          v[i][1] += dtfm * f[i][1];
        ...
        if (azflag) {
          dtfm = dtf / mass[type[i]];
          if(delta !=dt) {
           v[i][2] -= az*omega_rotate*cosine1;
          }
           v[i][2] += dtfm * f[i][2] + az*omega_rotate*cosine;
           x[i][2] += (dtv * v[i][2]);
        } else if (rmass) {
          dtfm = dtf / rmass[i];
          v[i][2] += dtfm * f[i][2];
...
void FixMovenve::final_integrate()
{
  double dtfm;
...
void FixMovenve::initial_integrate_respa(int vflag, int ilevel, int /*iloop*/)
{
...
void FixMovenve::final_integrate_respa(int ilevel, int /*iloop*/)
{
...
double FixMovenve::memory_usage()
{
...
void FixMovenve::write_restart(FILE *fp)
{
  int n = 0;
  double list[1];
  list[n++] = time_origin;
...
void FixMovenve::restart(char *buf)
{
  int n = 0;
...
void FixMovenve::grow_arrays(int nmax)
{
...
void FixMovenve::copy_arrays(int i, int j, int /*delflag*/)
{
...
void FixMovenve::set_arrays(int i)
{
  double theta;
  double *quat;
...
int FixMovenve::pack_exchange(int i, double *buf)
{
  int n = 0;
  buf[n++] = xoriginal[i][0];
  buf[n++] = xoriginal[i][1];
...
int FixMovenve::unpack_exchange(int nlocal, double *buf)
{
  int n = 0;
...
int FixMovenve::pack_restart(int i, double *buf)
{
  int n = 1;
  buf[n++] = xoriginal[i][0];
  buf[n++] = xoriginal[i][1];
...
void FixMovenve::unpack_restart(int nlocal, int nth)
{
...
int FixMovenve::maxsize_restart()
{
  return nrestart;
}
...
int FixMovenve::size_restart(int /*nlocal*/)
{
  return nrestart;
}
